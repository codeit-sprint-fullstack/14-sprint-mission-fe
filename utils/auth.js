...
export function validateEmail(value) {
  ...
  if (!emailPattern.test(email)) return AUTH_MESSAGES.emailInvalid;
  return '';
}
...
export function validatePassword(value) {
  ...
  if (password.length < 10) return AUTH_MESSAGES.passwordShort;
  return '';
}
...
export function validateSignupPassword(value) {
  ...
  if (!checks.forbidden) return AUTH_MESSAGES.passwordForbiddenSpecial;
  if (!checks.letters || !checks.number || !checks.special) return AUTH_MESSAGES.passwordWeak;
  return '';
}

export function validatePasswordConfirm(password, passwordConfirm) {
  const confirmValue = passwordConfirm.trim();
  if (!confirmValue) return AUTH_MESSAGES.passwordRequired;
  if (password.trim() !== confirmValue) return AUTH_MESSAGES.passwordMismatch;
  return '';
}
