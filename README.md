...
1. 개발자마다 다른 코드를 사용하지만 결국 결과물이 잘 구현되면 되는 것이다.
2. 그래도 '좋은 코드'라는 건 존재한다.

### 피드백

1. 코드를 정확히 이해하고 어떻게 작동하는지를 좀 더 공부하여 간결하고 깔끔하게 써보자. 죽은 코드를 만들지 말자.
2. 배우고 연습하는 과정이니 내가 잘 모르는 코드나 구현 방식으로 가장 어려운 길로 가보자.
